...
| `audit_requests_total` | Counter | `method`, `route`, `status`, `status_code`, `error_cause` | total (dimensionless count) | Total number of audit requests classified by method, route template, status, status_code, and bounded error_cause. | `src/observability/metrics-service.ts` | N/A |
| `audit_request_duration_seconds` | Histogram | `method`, `route`, `status`, `status_code`, `error_cause` | seconds | Duration of audit requests in seconds. | `src/observability/metrics-service.ts` | `0.005, 0.01, 0.05, 0.1, 0.25, 0.5, 1, 2.5, 5, +Inf` |
